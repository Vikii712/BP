                ...
                <div class="p-4 my-4 mx-8 max-w-fit bg-yellow-100 flex flex-col border-2 border-yellow-600 rounded-lg">
                    @if($lastBackup && !$showWarning)
                        <p class="text-gray-700 mb-2">
                            Posledná záloha: <span class="font-medium">{{ \Carbon\Carbon::parse($lastBackup->created_at)->format('d.m.Y H:i') }}</span>
                        </p>
                    @endif

                    <form method="POST" action="{{ route('backup.create') }}" class="inline-block">
                        @csrf
                        <button type="submit" class="bg-yellow-700 hover:bg-yellow-800 text-white px-4 py-2 rounded shadow">
                            Vytvoriť zálohu
                        </button>
                    </form>
                </div>
