<header class="fixed top-0 left-0 right-0 z-50 bg-blue-950 backdrop-blur-md px-6 pt-3 pb-2">
    <div class="flex items-center justify-between">
        ...
        <div class="flex items-center gap-4">
            ...
            <form action="{{ route('set-locale') }}" method="post" class="hidden md:flex items-center gap-1 bg-blue-900 rounded-full px-3 py-1">
                @csrf
...
<div id="mobile-menu" class="fixed left-0 w-full bg-blue-950 text-[var(--cream)] hidden z-40 overflow-y-auto">
    <div class="flex flex-col items-center gap-8 px-6">
        ...
        <div class="flex flex-row items-center gap-4 md:hidden">
            ...
            <form action="{{ route('set-locale') }}" method="post" class="flex items-center gap-1 bg-blue-900 rounded-full px-3 py-1">
                @csrf
        ...
        <ul class="flex flex-col gap-6 text-center font-medium w-full">
            <li><a href="{{ route('home') }}" class="flex justify-center items-center gap-3 hover:text-blue-300"><img src="{{ asset('images/domov.svg') }}" class="w-10 h-10" alt=""> Home</a></li>
